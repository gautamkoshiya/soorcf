...
<div class="container-fluid">
    <div class="row page-titles">
        <div class="col-md-5 align-self-center">
            <h2 class="text-themecolor">Proforma Invoice</h2>
        </div>
    ...
    <div class="row">
        <div class="card">
            <div class="card-body">
                <form action="#">
                    <div class="form-body">
                        <input type="hidden" name="invoice_id" id="invoice_id" value="{{ $proforma->id }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control custom-select customer_id select2 chosen-select" name="customer_id" id="customer_id" required>
                                        ...
                                        @foreach($customers as $customer)
                                            <option value="{{ $customer->id }}" {{ ($customer->id == $proforma->customer->id) ? 'selected':'' }}>{{ $customer->Name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control custom-select project_id select2 chosen-select" name="project_id" id="project_id" required>
                                        ...
                                        @foreach($projects as $project)
                                            <option value="{{ $project->id }}" {{ ($project->id == $proforma->project->id) ? 'selected':'' }}>{{ $project->Name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label class="control-label">Proforma Invoice Number</label>
                                    <input type="text" class="form-control PFINVNumber" name="PFINVNumber" id="PFINVNumber" value="{{ $proforma->PFINVNumber }}" readonly>
                                </div>
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label class="control-label">From date :- <span class="required">*</span></label>
                                            <input type="date" name="FromDate" id="FromDate" class="form-control" value="{{ $proforma->FromDate}}" placeholder="dd/mm/yyyy">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label class="control-label">Due date :- <span class="required">*</span></label>
                                            <input type="date" name="DueDate" id="DueDate" class="form-control" value="{{ $proforma->DueDate}}" placeholder="dd/mm/yyyy">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="table-responsive">
                            <table class="table color-bordered-table success-bordered-table">
                                ...
                                <tbody>
                                @foreach($proforma_invoice_details as $details)
                        ...
                        <div class="row">
                            <div class="col-md-10">
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <div class="form-group">
                                            <textarea name="TermsAndCondition" id="TermsAndCondition" cols="30" rows="5" class="form-control" style="width: 100%" placeholder="Terms And Condition">{{ $proforma->TermsAndCondition }}</textarea>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <label>Customer Note :- </label>
                                        <div class="form-group">
                                            <textarea name="CustomerNote" id="CustomerNote" cols="30" rows="5" class="form-control" style="width: 100%" placeholder="Customer Note">{{ $proforma->CustomerNote }}</textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-2">
                                        ...
                                        <div class="form-group">
                                            <select name="IsNeedStampOrSignature" class="form-control IsNeedStampOrSignature" id="IsNeedStampOrSignature">
                                                <option value="0" {{ ($proforma->IsNeedStampOrSignature == 0) ? 'selected':'' }}>No</option>
                                                <option value="1" {{ ($proforma->IsNeedStampOrSignature == 1) ? 'selected':'' }}>YES</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-2">
                                ...
                                <p>Discount(AED): <input type="number" value="{{$proforma->discount}}" class="form-control discount">
                                ...
                                <div class="form-actions">
                                    ...
                                    <a href="{{ route('proforma_invoices.index') }}" class="btn btn-danger"><i class="fa fa-times" aria-hidden="true"></i> Cancel</a>
                                </div>
                            </div>
                        </div>
                    </div>
...
$(document).ready(function () {
    ...
    $(document).on("click",'.addRow', function () {
        ...
        if(validateRow(currentRow))
        {
            ...
            var discount = currentRow.find('.discount').val();
            {
                ...
                var html = '';
                html += '<tr>';
                html += '<td><select name="Product_id" class="product_id form-control"><option value="">Product</option>@foreach($products as $product)<option value="{{ $product->id }}">{{ $product->Name }}</option>@endforeach</select></td>';
    ...
    $('#submit').click(function () {
        if(validateForm() && validateAllRows())
        {
            ...
            if (insert.length > 0)
            {
                ...
                $.ajax({
                    url: "{{ URL('ProformaInvoiceUpdate') }}/" + Id,
                    type: "post",
                    data: Datas,
                    success: function (result) {
                        if (result===true) {
                            alert('Proforma Invoice Updated Succesfully...');
                            window.location.href = "{{ route('proforma_invoices.index') }}";
                        } else {
    ...
    $('.customer_id').change(function () {
        ...
        {
            $.ajax({
                url: "{{ URL('quotationCustomerDetails') }}/" + Id,
                type: "get",
                dataType: "json",
                success: function (result) {
                    if (result !== false)
                    {
                        ...
                        $('#Mobile').text(result.data[0].Mobile);
                        $('#Email').text(result.data[0].Email);
                        $('#TRN').text(result.data[0].TRNNumber);
                    } else {
