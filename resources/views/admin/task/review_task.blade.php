...
@section('content')
        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <h2 class="card-title">Review Task</h2>
                            <div class="table-responsive m-t-40">
                                <table id="task_review_table" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                    <thead>
                                    <tr>
                                        <th>SR#</th>
                                        <th>Task</th>
        ...
        $(document).ready(function () {
            $('#delete_row_submit').click(function (event) {
                if (validateForm()) {
                    ...
                    $.ajax({
                        url: "{{ URL('task_delete_post') }}",
                        type: "post",
                        data: {deleteDescription:deleteDescription,row_id:_row_id},
                        success: function (result) {
                            if (result === true) {
                                window.location.href = "{{ route('review_task') }}";
                            } else {
    ...
    <script>
        $(document).ready(function () {
            $('#task_review_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    "url": "{{ url('get_review_task') }}",
                    "dataType": "json",
                    "type": "POST",
