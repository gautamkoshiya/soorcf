...
<div class="container-fluid">
    <div class="row page-titles">
        <div class="col-md-5 align-self-center">
            <h2 class="text-themecolor">LPO</h2>
        </div>
    ...
    <div class="row">
        <div class="card">
            <div class="card-body">
                <form action="#">
                    <div class="form-body">
                        <input type="hidden" name="lpo_id" id="lpo_id" value="{{ $lpo->id }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control custom-select customer_id select2 chosen-select" name="customer_id" id="customer_id" required>
                                        ...
                                        @foreach($customers as $customer)
                                            <option value="{{ $customer->id }}" {{ ($customer->id == $lpo->supplier->id) ? 'selected':'' }}>{{ $customer->Name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control custom-select project_id select2 chosen-select" name="project_id" id="project_id" required>
                                        ...
                                        @foreach($projects as $project)
                                            <option value="{{ $project->id }}" {{ ($project->id == $lpo->project->id) ? 'selected':'' }}>{{ $project->Name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control LPONumber" name="LPONumber" id="LPONumber" value="{{ $lpo->LPONumber }}" readonly>
                                </div>
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label class="control-label">From date :- <span class="required">*</span></label>
                                            <input type="date" name="FromDate" id="FromDate" class="form-control" value="{{ $lpo->FromDate}}" placeholder="dd/mm/yyyy">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label class="control-label">Due date :- <span class="required">*</span></label>
                                            <input type="date" name="DueDate" id="DueDate" class="form-control" value="{{ $lpo->DueDate}}" placeholder="dd/mm/yyyy">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="table-responsive">
                            <table class="table color-bordered-table success-bordered-table">
                                ...
                                <tbody>
                                @foreach($lpo_details as $details)
                        ...
                        <div class="row">
                            <div class="col-md-10">
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <div class="form-group">
                                            <textarea name="TermsAndCondition" id="TermsAndCondition" cols="30" rows="5" class="form-control" style="width: 100%" placeholder="Terms And Condition">{{ $lpo->TermsAndCondition }}</textarea>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <label>Customer Note :- </label>
                                        <div class="form-group">
                                            <textarea name="CustomerNote" id="CustomerNote" cols="30" rows="5" class="form-control" style="width: 100%" placeholder="Customer Note">{{ $lpo->CustomerNote }}</textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-md-2">
                                        ...
                                        <div class="form-group">
                                            <select name="IsNeedStampOrSignature" class="form-control IsNeedStampOrSignature" id="IsNeedStampOrSignature">
                                                <option value="0" {{ ($lpo->IsNeedStampOrSignature == 0) ? 'selected':'' }}>No</option>
                                                <option value="1" {{ ($lpo->IsNeedStampOrSignature == 1) ? 'selected':'' }}>YES</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-2">
                                ...
                                <p>Discount(AED): <input type="number" value="{{$lpo->discount}}" class="form-control discount">
                                ...
                                <div class="form-actions">
                                    ...
                                    <a href="{{ route('lpos.index') }}" class="btn btn-danger"><i class="fa fa-times" aria-hidden="true"></i> Cancel</a>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    $('#submit').click(function () {
        if(validateForm() && validateAllRows())
        {
            ...
            if (insert.length > 0)
            {
                ...
                var Id = $('#lpo_id').val();
                $.ajax({
                    url: "{{ URL('lpoUpdate') }}/" + Id,
                    type: "post",
                    data: Datas,
                    success: function (result) {
                        if (result===true) {
                            alert('LPO Updated Succesfully...');
